...
const Dashboard = () => {
  const { chemoSessions, medications, journalEntries } = useData();
  ...
  const latestJournalEntry = journalEntries.length > 0 
  ...
  return (
    <div>
      <div className="mb-8 animate-fade-in">
        ...
        <p className="text-gray-600">Treatment started on {formatDate(new Date('2025-05-09'))}</p>
      </div>
          ...
          <Card className="h-full card-hover">
            ...
            <CardContent>
              <ul className="space-y-2">
                {medications.map((med) => (
                  <li key={med.id} className="p-2 bg-white rounded border border-gray-100">
